...
export function MobileTopbar({ onOpenUpload }: MobileTopbarProps) {
  ...
  return (
    <>
      ...
      >
        {/* Left Area: Drawer Toggle + Contextual Page Info */}
        <div className="flex items-center gap-2 min-w-0">
          ...
          {isDeepRoute ? (
            <button
              ...
              onClick={() => navigate(-1)}
              aria-label="Go back"
              className="flex items-center gap-1.5 p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/40 transition-colors"
            >
      ...
      <div className="flex items-center gap-2 shrink-0">
        <Button
          ...
          className="h-8 px-2.5 rounded-xl font-medium text-xs gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90 shadow-2xs transition-all active:scale-95"
        >
