            ...
            <div className="rounded-3xl border border-border/80 bg-card p-6 sm:p-8 shadow-sm space-y-6 text-left">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1 text-xs">
                  ...
                  <span className="px-3 py-1 rounded-xl bg-primary text-primary-foreground font-medium shrink-0 shadow-2xs">
                    Government
                  </span>
                  <span className="px-3 py-1 rounded-xl border border-border/60 bg-muted/40 text-muted-foreground hover:text-foreground shrink-0">
                    Student
                  </span>
                  <span className="px-3 py-1 rounded-xl border border-border/60 bg-muted/40 text-muted-foreground hover:text-foreground shrink-0">
                    PDF
              ...
              {/* Tag Showcase Ribbon (Each tag shows visible text) */}
              <div className="pt-2 pb-1 border-y border-border/50 flex items-center justify-between gap-2 overflow-x-auto">
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  {tags.map((t) => (
                    <span
                      key={t.name}
                      className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-md border ${t.color}`}
                    >
                      {t.name}
                    </span>
                  ))}
                ...
                {mockResults.map((doc) => (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="p-2.5 rounded-xl bg-card border border-border/70 text-foreground shrink-0 shadow-2xs">
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-start sm:self-center">
                      <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-md border ${doc.tagClass}`}>
                        {doc.tag}
                      </span>
